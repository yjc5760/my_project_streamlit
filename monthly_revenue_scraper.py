# monthly_revenue_scraper.py (整合重試機制與強制 Secrets 版)
import os
import sys
import requests
import pandas as pd
...
def fetch_stock_data(url: str, headers: dict, table_id: str) -> pd.DataFrame | None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"✅ 第 {attempt} 次嘗試連線成功，正在解析表格...")
            break  # 成功抓取則跳出迴圈

        except requests.exceptions.RequestException as e:
            print(f"❌ 請求失敗 第 {attempt} 次嘗試: {e}")
            if attempt < retries:
                print(f"{delay} 秒後重試...")
                time.sleep(delay)
            else:
...
def scrape_goodinfo() -> pd.DataFrame | None:
    ...
    嚴格依賴 Streamlit secrets 注入的 Cookie。
    """
    ...
    if not cookie:
        print("❌ 錯誤：找不到環境變數 'GOODINFO_COOKIE_MONTHLY'。請確認已在 Streamlit Secrets 中設定。")
        return None
