...
"""
    ...
    if upper_bound_df.empty:
        # 如果找不到更大的時間（例如剛好在 13:30），則使用最後一個因子
    ...
    # 線性內插計算
    interpolated_factor = f1 + (now_sec - t1_sec) * (f2 - f1) / (t2_sec - t1_sec)
    
    return interpolated_factor


def scrape_yahoo_stock_rankings(url):
    """
    使用 Requests 和 BeautifulSoup 抓取 Yahoo 股市的股票漲幅排行榜資料。
    """
    ...
    try:
        ...
        res.raise_for_status() # 如果請求失敗 (e.g., 404, 500)，會拋出異常
        res.encoding = 'utf-8'

        soup = BeautifulSoup(res.text, 'html.parser')
        
        # 每一支股票的資料都包在一個 <li> 標籤中
        ...
        for i, row in enumerate(rows):
            try:
                ...
                change = pd.to_numeric(data_containers[1].text.strip(), errors='coerce')
        ...
        if 'Volume (Shares)' in df.columns:
            # 確保 'Volume (Shares)' 欄位是數值型態，以進行計算
        ...
        if 'Stock Symbol' in df.columns:
            ...
            df['Stock Symbol'] = pd.to_numeric(df['Stock Symbol'], errors='coerce')
            df['Stock Symbol'] = df['Stock Symbol'].fillna(0).astype('int64')
...
if __name__ == '__main__':
    # ==================== 主要修改處 ====================
    # 將 exchange=TAI (上市) 改為 exchange=TWO (上櫃)
    url = "https://tw.stock.yahoo.com/rank/change-up?exchange=TWO"
    # ===================================================

    yahoo_stock_df = scrape_yahoo_stock_rankings(url)

    if yahoo_stock_df is not None:
        # ===== 修改說明文字，使其符合「上櫃」情境 =====
        print("\n抓取到的 Yahoo 股市-上櫃漲幅排行榜資料：")
        # ===============================================

        # 為了更清晰地顯示結果，我們只顯示最重要的幾個欄位
        display_columns = ['Rank', 'Stock Symbol', 'Stock Name', 'Price', 'Volume (Shares)', 'Factor', 'Estimated Volume']
        # 確保所有要顯示的欄位都存在
        existing_columns = [col for col in display_columns if col in yahoo_stock_df.columns]
        print(yahoo_stock_df[existing_columns].head(10))

# --- END OF FILE yahoo_stock.py ---