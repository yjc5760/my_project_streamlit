"""
stock_information_plot.py
繪製個股月營收趨勢圖與主要股東持股比例圖。
所有函式統一回傳 (fig, error_msg) tuple：
  - 成功：(go.Figure, None)
  - 失敗：(None, str)
"""

import pandas as pd
import numpy as np
import os
import requests
import twstock
import time
import random
from bs4 import BeautifulSoup
from io import StringIO

import plotly.graph_objects as go
from plotly.subplots import make_subplots

from utils import get_stock_code


def _goodinfo_get(url: str, cookie_keys: list[str], stock_code: str) -> tuple[str | None, str | None]:
    """
    共用 Goodinfo HTTP 請求邏輯。
    cookie_keys: 依優先序嘗試的環境變數名稱清單。
    回傳 (html, error_msg)。
    """
    cookie = ""
    for key in cookie_keys:
        cookie = os.getenv(key, "")
        if cookie:
            break

    headers = {
        "User-Agent": (
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
            "AppleWebKit/537.36 (KHTML, like Gecko) "
            "Chrome/122.0.0.0 Safari/537.36"
        ),
        "Referer": f"https://goodinfo.tw/tw/StockDetail.asp?STOCK_ID={stock_code}",
    }
    if cookie:
        headers["Cookie"] = cookie

    time.sleep(random.uniform(0.5, 1.2))

    try:
        r = requests.get(url, headers=headers, timeout=15)
        if r.status_code == 403:
            return None, f"Goodinfo 拒絕存取（HTTP 403）。請確認 Streamlit Secrets 中的 Cookie 是否有效。"
        r.raise_for_status()
        r.encoding = "utf-8"
        html = r.text
        if "<title>Goodinfo! 台灣股市資訊網 - 首頁</title>" in html:
            return None, "Cookie 已失效，被導回首頁。請至 Streamlit Secrets 更新 Cookie。"
        return html, None
    except requests.exceptions.Timeout:
        return None, "連線 Goodinfo 逾時（15 秒），請稍後再試。"
    except requests.exceptions.RequestException as e:
        return None, f"網路請求失敗：{e}"


def plot_stock_major_shareholders(stock_identifier):
    """
    根據股票代碼或名稱，爬取並繪製主要股東持股比例圖。
    回傳 (go.Figure, None) 或 (None, error_str)。
    """
    stock_code = get_stock_code(stock_identifier)
    if not stock_code:
        return None, f"找不到股票代碼：{stock_identifier}"

    url = f"https://goodinfo.tw/tw/StockBigHolder.asp?STOCK_ID={stock_code}"
    html, err = _goodinfo_get(url, ["GOODINFO_COOKIE_MY_STOCK", "GOODINFO_COOKIE_MONTHLY"], stock_code)
    if html is None:
        return None, err

    try:
        soup = BeautifulSoup(html, "lxml")
        table = soup.find("table", {"id": "tblStockHolder"})
        if table is None:
            return None, f"找不到 {stock_code} 的股東資料表格（tblStockHolder），網站結構可能已變更。"

        dfs = pd.read_html(StringIO(str(table)))
        if not dfs:
            return None, f"{stock_code} 股東表格解析結果為空。"

        df = dfs[0]
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(-1)
        df.columns = df.columns.astype(str).str.replace(r"\s+", "", regex=True)

        holder_col = next((c for c in df.columns if "股東" in c or "名稱" in c), None)
        ratio_col = next((c for c in df.columns if "持股" in c or "比例" in c or "%" in c), None)

        if not holder_col or not ratio_col:
            return None, f"無法識別欄位（持股比例欄），目前欄位：{df.columns.tolist()}"

        df_plot = df[[holder_col, ratio_col]].copy()
        df_plot[ratio_col] = pd.to_numeric(df_plot[ratio_col], errors="coerce")
        df_plot = df_plot.dropna(subset=[ratio_col]).head(15)

        if df_plot.empty:
            return None, f"{stock_code} 股東資料轉換後為空。"

        stock_info = twstock.codes.get(stock_code)
        stock_name = stock_info.name if stock_info else stock_code

        fig = go.Figure(go.Bar(
            x=df_plot[ratio_col],
            y=df_plot[holder_col],
            orientation="h",
            marker_color="steelblue",
            text=df_plot[ratio_col].apply(lambda x: f"{x:.2f}%"),
            textposition="outside",
        ))
        fig.update_layout(
            title=f"{stock_name} ({stock_code}) 主要股東持股比例",
            xaxis_title="持股比例 (%)",
            yaxis=dict(autorange="reversed"),
            height=500,
            margin=dict(l=200, r=50, t=60, b=50),
        )
        return fig, None

    except Exception as e:
        return None, f"繪製 {stock_code} 股東圖時發生錯誤：{type(e).__name__}: {e}"


def plot_stock_revenue_trend(stock_identifier):
    """
    根據股票代碼或名稱，爬取 Goodinfo 月營收頁面並繪製趨勢圖。
    回傳 (go.Figure, None) 或 (None, error_str)。

    Goodinfo 月營收為 pivot table（列=年份、欄=1月～12月）。
    解析流程：
      1. 精準定位含月份欄位的表格。
      2. melt 成長格式，以「民國年/月」為時間軸。
      3. 柱狀圖（單月營收，依年份分色）+ 年增率折線（若有）。
    """
    stock_code = get_stock_code(stock_identifier)
    if not stock_code:
        return None, f"找不到股票代碼：{stock_identifier}"

    url = f"https://goodinfo.tw/tw/StockMonthlyRevenue.asp?STOCK_ID={stock_code}"
    html, err = _goodinfo_get(url, ["GOODINFO_COOKIE_MONTHLY", "GOODINFO_COOKIE_MY_STOCK"], stock_code)
    if html is None:
        return None, err

    try:
        MONTH_LABELS = [f"{m}月" for m in range(1, 13)]

        soup = BeautifulSoup(html, "lxml")

        # 精準定位含月份欄位的表格
        table_tag = (
            soup.find("table", id="tblDetail") or
            soup.find("table", id="tblStockRevenue") or
            soup.find("table", id="tblRevenue")
        )
        if table_tag:
            dfs = pd.read_html(StringIO(str(table_tag)), flavor="lxml")
        else:
            dfs = pd.read_html(StringIO(html), flavor="lxml")

        if not dfs:
            return None, f"找不到 {stock_code} 的月營收資料（頁面無任何表格）。"

        # 挑出包含月份欄位的表格
        df_raw = None
        for d in dfs:
            cols = d.columns.get_level_values(-1) if isinstance(d.columns, pd.MultiIndex) else d.columns
            cols_flat = [str(c).replace(" ", "") for c in cols]
            if any(m in cols_flat for m in MONTH_LABELS):
                df_raw = d
                break

        if df_raw is None:
            # 退回最大表格
            df_raw = max(dfs, key=lambda d: d.shape[0] * d.shape[1])
            # 最後確認是否有月份欄
            if isinstance(df_raw.columns, pd.MultiIndex):
                cols_flat = [str(c).replace(" ", "") for c in df_raw.columns.get_level_values(-1)]
            else:
                cols_flat = [str(c).replace(" ", "") for c in df_raw.columns]
            if not any(m in cols_flat for m in MONTH_LABELS):
                all_cols = df_raw.columns.get_level_values(-1).tolist() if isinstance(df_raw.columns, pd.MultiIndex) else df_raw.columns.tolist()
                return None, (
                    f"{stock_code} 頁面中找不到月份欄位（1月～12月）。"
                    f" 可能 Cookie 權限不足或頁面結構已變更。"
                    f" 目前最大表格欄位：{all_cols[:10]}"
                )

        # 展平 MultiIndex，標準化欄名
        if isinstance(df_raw.columns, pd.MultiIndex):
            df_raw.columns = df_raw.columns.get_level_values(-1)
        df_raw.columns = df_raw.columns.astype(str).str.replace(r"\s+", "", regex=True)

        # 辨識年度欄
        year_col = next(
            (c for c in df_raw.columns
             if c in ("年度", "年份", "會計年度") or ("年" in c and len(c) <= 4)),
            df_raw.columns[0]
        )

        month_cols = [c for c in df_raw.columns if c in MONTH_LABELS]
        if not month_cols:
            month_cols = [c for c in df_raw.columns
                          if c != year_col and c.isdigit() and 1 <= int(c) <= 12]

        if not month_cols:
            return None, f"無法識別 {stock_code} 月份欄位，目前欄位：{df_raw.columns.tolist()}"

        # 年增率欄（可選）
        yoy_col = next(
            (c for c in df_raw.columns
             if "年增" in c and "%" in c and "前" not in c and "累" not in c),
            None
        )

        # 清理資料
        df_work = df_raw[[year_col] + month_cols + ([yoy_col] if yoy_col else [])].copy()
        df_work[year_col] = pd.to_numeric(df_work[year_col], errors="coerce")
        df_work = df_work.dropna(subset=[year_col])
        if df_work.empty:
            return None, f"{stock_code} 年份欄清理後無有效資料。"
        df_work[year_col] = df_work[year_col].astype(int)

        for col in month_cols + ([yoy_col] if yoy_col else []):
            df_work[col] = pd.to_numeric(df_work[col], errors="coerce")

        # melt 成長格式
        id_vars = [year_col] + ([yoy_col] if yoy_col else [])
        df_long = df_work.melt(id_vars=id_vars, value_vars=month_cols,
                               var_name="月份", value_name="單月營收")
        df_long = df_long.dropna(subset=["單月營收"])

        if df_long.empty:
            return None, f"{stock_code} 月營收資料 melt 後全為空值。"

        df_long["月份數"] = df_long["月份"].str.replace("月", "").astype(int)
        df_long = df_long.sort_values([year_col, "月份數"])
        df_long["期間"] = df_long[year_col].astype(str) + "/" + df_long["月份數"].astype(str)

        # 繪圖
        stock_info = twstock.codes.get(stock_code)
        stock_name = stock_info.name if stock_info else stock_code

        has_yoy = yoy_col and not df_work[yoy_col].isna().all()

        if has_yoy:
            fig = make_subplots(
                rows=2, cols=1,
                shared_xaxes=True,
                vertical_spacing=0.08,
                row_heights=[0.65, 0.35],
                subplot_titles=["單月營收 (千元)", "年增率 (%)"]
            )
        else:
            fig = make_subplots(rows=1, cols=1)

        colors = ["#3b82f6", "#ef4444", "#10b981", "#f59e0b",
                  "#8b5cf6", "#ec4899", "#06b6d4", "#84cc16"]

        for i, year in enumerate(sorted(df_long[year_col].unique())):
            sub = df_long[df_long[year_col] == year]
            fig.add_trace(
                go.Bar(name=f"{year}年", x=sub["期間"], y=sub["單月營收"],
                       marker_color=colors[i % len(colors)], opacity=0.85),
                row=1, col=1
            )

        if has_yoy:
            yoy_map = df_work.set_index(year_col)[yoy_col].to_dict()
            df_long["年增率"] = df_long[year_col].map(yoy_map)
            fig.add_trace(
                go.Scatter(name="年增率", x=df_long["期間"], y=df_long["年增率"],
                           mode="lines+markers",
                           line=dict(color="#f97316", width=2),
                           marker=dict(size=5)),
                row=2, col=1
            )
            fig.add_hline(y=0, line_dash="dash", line_color="gray",
                          opacity=0.5, row=2, col=1)

        fig.update_layout(
            title=f"{stock_name} ({stock_code}) 月營收趨勢",
            xaxis_title="期間（民國年/月）",
            barmode="group",
            height=560 if has_yoy else 420,
            legend=dict(orientation="h", yanchor="bottom", y=1.02,
                        xanchor="right", x=1),
        )
        fig.update_yaxes(title_text="營收 (千元)", row=1, col=1)
        if has_yoy:
            fig.update_yaxes(title_text="年增率 (%)", row=2, col=1)

        print(f"成功繪製 {stock_code} 月營收圖，共 {len(df_long)} 筆資料。")
        return fig, None

    except Exception as e:
        return None, f"繪製 {stock_code} 月營收圖時發生錯誤：{type(e).__name__}: {e}"
