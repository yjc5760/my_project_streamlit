...
try:
    ...
    from stock_analyzer import analyze_stock, create_chart_from_result
...
@st.cache_data(ttl=3600) # 個股分析資料快取1小時（只快取純資料，不含 Figure）
def cached_analyze_stock(stock_id):
    # 【Bug 1 修正】：analyze_stock 已不再回傳 Figure，確保 cache_data 可序列化
    return analyze_stock(stock_id)

def get_stock_chart(stock_id: str) -> go.Figure | None:
    """
    【Bug 1+4 修正】：從快取的純資料結果重建 Figure，不使用 cache_data。
    每次顯示圖表時呼叫，Figure 物件在 session 內重建，不存入快取。
    """
    result = cached_analyze_stock(stock_id)
    if result.get('status') == 'success':
        return create_chart_from_result(result)
    return None
...
def process_ranking_analysis(stock_df: pd.DataFrame) -> list:
    ...
    try:
        ...
        # 【Bug 4 修正】：ThreadPoolExecutor 與 @st.cache_data 有執行緒競態問題。
        # 改為直接呼叫非快取的 analyze_stock，完成後再寫入 session_state 當作快取。
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_stock = {
                executor.submit(analyze_stock, str(stock_info['Stock Symbol']).strip()): stock_info
            ...
            for i, future in enumerate(as_completed(future_to_stock)):
                ...
                try:
                    ...
                    if analysis_result['status'] == 'success':
                        ...
                        # 【Bug 5 修正】：Yahoo 回傳的 Estimated Volume 單位是「股」，需除以 1000 換算成「張」
                        estimated_volume_lots = stock_info.get('Estimated Volume', 0) / 1000
                        ...
                        if pd.notna(estimated_volume_lots) and pd.notna(avg_vol_5_lots) and avg_vol_5_lots > 0 and estimated_volume_lots > (2 * avg_vol_5_lots):
                            result_item.update({
                                'error': None,
                                # 【Bug 1 修正】：不再存 Figure，改存 stock_id 供後續重建圖表
                                'stock_id_for_chart': str(stock_info['Stock Symbol']).strip(),
...
def display_concentration_visualization(df: pd.DataFrame):
    ...
    viz_df['_K'] = viz_df['KD'].apply(parse_k) if 'KD' in viz_df.columns else pd.Series([None] * len(viz_df), index=viz_df.index)
    viz_df['_D'] = viz_df['KD'].apply(parse_d) if 'KD' in viz_df.columns else pd.Series([None] * len(viz_df), index=viz_df.index)
    viz_df['_I'] = viz_df['I值'].apply(parse_i) if 'I值' in viz_df.columns else pd.Series([None] * len(viz_df), index=viz_df.index)
                ...
                **篩選條件：**
                ...
                for _, stock in filtered_stocks.iterrows():
                    ...
                    with st.expander(f"查看 {stock_name} ({stock_code}) 的技術分析圖"):
                        # 【Bug 1 修正】：用 get_stock_chart 重建 Figure，不從快取取 Figure
                        chart_fig = get_stock_chart(stock_code)
                        if chart_fig:
...
def display_goodinfo_results():
    ...
    if scraped_df is not None and not scraped_df.empty:
        ...
        **篩選條件 (來自 Goodinfo 自訂篩選):**
        ...
        display_columns = [
            # 【Bug 2 修正】：欄位名稱需與 scraper.py 移除空白後的實際輸出一致
            '代碼', '名稱', 'KD', 'I值', '市場', '股價日期',
            '成交', '漲跌價', '漲跌幅', '成交張數'
        ]
        ...
        for result in yahoo_results:
            if not result.get('error'):
                ...
                with st.expander(f"查看 {stock_name} ({stock_symbol}) 的技術分析圖"):
                    # 【Bug 1 修正】：從 stock_id 重建 Figure，不從快取取 Figure
                    chart_fig = get_stock_chart(result.get('stock_id_for_chart', str(stock_symbol)))
                    if chart_fig:
                        st.plotly_chart(chart_fig, use_container_width=True)
                    else:
                        st.error(f"無法生成 {stock_name} 的技術分析圖")
